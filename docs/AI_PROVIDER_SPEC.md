...
> huggingface closed the deferral); `retry-ai` Stage 17; creation-time
> key proof + per-run disclosure + retry bucket Stage 21). This file fixes
...
## 5. Credential lifecycle (vault Stage 12; adapters Stage 18; create proof Stage 21)

> As-built Stage 21: creation is SHAPE + LIVE proof (`validate_credentials`) before
> storage — a rejected/unreachable key returns `400 validation_error` with adapter-curated
> user-safe copy and stores nothing. Successful creates stamp `last_test_status=ok` +
> `last_tested_at` because the key was just proven. As-built Stage 14: TEST runs the full
> decrypt → adapter → sanitize → record flow through `resolve_adapter` — LIVE for the four
> implemented providers (`GET /v1/models` / `GET /v1beta/models` probe + curated model list
> on success). As-built Stage 18: LIVE for all six (Anthropic probes `GET /v1/models`, HF
> probes the router's `GET /v1/models`); the deterministic `200 {ok:false}` + guidance
> message stays as the defensive branch for adapter-less (future) providers, leaving any
> existing `last_test_*` verdict untouched.
...
2. **Post-registration nudge:** after verification, inform that core detection works
   ...
   Settings add-provider copy (Stage 21) also states that keys are provider-verified
...
3. **Result-page AI states (IMPLEMENTED Stage 14 as `AiOverviewSection` + the
   ...
   the user-safe error verbatim + the deterministic-unaffected note and, where
   the parent can refresh the detail, the Stage 17 `Retry AI` button; `skipped`
   shows a neutral one-liner; `ok` shows the labeled overview with "Generated by
   {provider}" attribution plus Stage 21 what-was-sent disclosure copy. Analyzer
   + upload forms carry the opt-in checkbox + Settings link; history/dashboard
   are untouched (their summary shape carries no AI fields — no contract churn
   for decoration).
...
Explicit retry (IMPLEMENTED Stage 17; dedicated bucket Stage 21):
`POST /analysis/{id}/retry-ai` re-runs the whole step above through the SAME service
(reset → re-read → enhance — never a parallel path). Any prior `ai_status` is accepted
(a retry is always explicit, §9's "plus explicit retries" allowance); the reset drops
AI-stamped rewrites first so a failed retry can't strand stale `ok` output. The report's
failed card carries the "Try again" button; `unconfigured`/`skipped`/`ok` states get no
button (their next actions are Settings / nothing / nothing). Retry uses its own
per-user bucket (`RATE_LIMIT_AI_RETRY_PER_MINUTE`, default 10), separate from provider
TEST.
...
- Settings + result pages disclose: which provider + model was used, what text was sent
  ...
  choice). As-built Stage 21 adds per-run UI copy on successful AI overviews:
  overview uses finding summaries; rewrites use only flagged requirement excerpts;
  provider retention follows the provider's policy.
...
- Per-user AI rate limits: provider TEST has a dedicated bucket since Stage 12
  (`RATE_LIMIT_AI_TEST_PER_MINUTE`, default 10); explicit `retry-ai` has a dedicated
  bucket since Stage 21 (`RATE_LIMIT_AI_RETRY_PER_MINUTE`, default 10). Distributed
  storage and Turnstile remain roadmap-22.
