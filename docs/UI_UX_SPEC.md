...
reasoning (per-issue AI explanations still future — no provider call produces
them yet; Stage 14 AI surfaces are the report-level overview + per-requirement
rewrites, §13). No black boxes.
...
- The report carries ONE AI outcome block (`AiOverviewSection`, directly under
  ...
  the backend's user-safe `ai_error` VERBATIM + the deterministic-unaffected
  note + a "Try again" button (Stage 17: the endpoint exists now — the
  button POSTs `retry-ai` with pending + code-mapped errors, and the parent
  silently re-reads the detail on success; no wiring, no button — §9 still
  bans fake controls); `unconfigured` = dashed empty state + "Open Settings" CTA;
  `skipped` = neutral one-liner, no CTA. All four keep deterministic findings
  fully visible — AI never displaces the authoritative result.
- Rewrites render INSIDE their requirement card, BELOW the original, labeled
  "AI-suggested rewrite" (or "Suggested rewrite" for non-AI sources) —
  additive-only, originals immutable. Model output renders as PLAIN TEXT
